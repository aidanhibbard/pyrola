...
fn copy_recursive(from: &Path, to: &Path) -> Result<(), String> {
  ...
  if meta.is_dir() {
    fs::create_dir_all(to).map_err(|error| format!("Failed to copy leftover graph dir: {error}"))?;
    for entry in fs::read_dir(from)
      .map_err(|error| format!("Failed to copy leftover graph dir: {error}"))?
    {
      let entry = entry.map_err(|error| format!("Failed to copy leftover graph dir: {error}"))?;
