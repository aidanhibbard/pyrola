...
pub(crate) fn apply_replacements(content: &str, replacements: &[FsEditReplacement]) -> Result<String, String> {
...
pub(crate) fn apply_patch_operations(project_root: &str, patch: &str) -> Result<Vec<FileDiff>, String> {
  let root = canonical_project_root(project_root)?;
  let operations = parse_patch(patch)?;
  let mut diffs = Vec::new();
