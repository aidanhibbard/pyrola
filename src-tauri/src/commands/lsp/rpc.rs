...
use super::resolve::{
  active_project_root, build_initialization_options, find_server_for_extension, inject_vue_tsdk_arg,
  load_effective_servers, resolve_lsp_command, LspServerEntry,
};
...
pub(crate) async fn write_lsp_message(stdin: &mut ChildStdin, body: &serde_json::Value) -> Result<(), String> {
...
pub(crate) async fn read_lsp_message(reader: &mut BufReader<ChildStdout>) -> Result<serde_json::Value, String> {
