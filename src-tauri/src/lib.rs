...
use commands::{
  append_chat_line, codegraph_cli, codegraph_store_stat, delete_graph, list_graphs,
  config_exists, create_chat, delete_chat, delete_secret, fork_chat, truncate_chat_log,
  ...
  write_temp_bytes, write_temp_handoff, write_text_file, file_checkpoint_capture,
  ...
  read_json_file, read_lsp_config, read_mcp_config, read_settings, registry_add_project,
  open_project_at_path, open_project_at_path_command,
...
pub fn run_with_launch_path(launch_path: Option<String>) {
  ...
  let builder = builder
    ...
    .setup(move |app| {
      ...
      #[cfg(feature = "cef")]
      {
        // Message loop: macOS uses external_message_pump + OnScheduleMessagePumpWork
        // (GCD main-queue timers). Do not start a background thread that calls
        // do_message_loop_work via run_on_main_thread; that re-enters NSApplication
        // and blocks the Tauri main thread.
        if let Err(error) = crate::commands::browser_cef::warm_init(app.handle()) {
          log::warn!("CEF warm-init failed: {error}");
        }
      }
  ...
  let builder = builder.invoke_handler(tauri::generate_handler![
    ...
    browser_cef_create,
    browser_cef_destroy,
    browser_cef_navigate,
    browser_cef_resize,
    browser_cef_focus,
    browser_cef_get_url,
    browser_cef_get_title,
    browser_cef_can_go_back,
    browser_cef_can_go_forward,
    browser_cef_go_back,
