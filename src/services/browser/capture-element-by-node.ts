...
const resolveObjectId = async (
  ...
  if (typeof objectId !== 'string' || objectId.length === 0) {
    throw new Error('Failed to resolve DOM node for the selected element')
  }
  return objectId
}

const captureElementByBackendNodeId = async (
  client: CdpClient,
  sessionId: string,
  backendNodeId: number,
): Promise<BrowserElementSelection> => {
  const objectId = await resolveObjectId(client, sessionId, backendNodeId)
  ...
  const matchedCss = await matchedStylesForNode(client, sessionId, objectId)
...
export default captureElementByBackendNodeId
